...
Upon successful execution of the script, another scheduled task is triggered to verify if all stock are loaded and then transfer the data files from my **local machine** to the designated **raw** folder in **Azure Blob Storage** using **AZCOPY**. Raw data often refers to unprocessed or minimally processed data that is directly collected from sources. In this context, the **raw** folder acts as the starting point for storing stock data before any further processing or analysis.
...
### Daily Data Processing ("daily_process_stocks"):
...
### Stocks information and prices ("daily_stocks-prices_sql_tables"):
...
### GARCH(1,1) Volatility (daily_volatility_sql_tables):
...
### Monte-Carlo simulation (daily_monte-carlo_simulation_sql_tables)
...
This illustrates the price distribution over a period of `250` days.
...
This notebook aims to combine different tables to create a comprehensive dataset for visualization. It includes stock information, prices, returns, volatility, and simulated prices, with a designated "stock" column indicating the respective stock tag.

here is an example of the joined returns table:

![The original returns table](original_returns.png)

![The join returns table](table.png)

An illustrative SQL code example is provided for table joining in the [notebook](https://github.com/mus514/ML_Pipeline_Hub/blob/main/Daily-process_Notebooks/sql_join_tables.ipynb).

```{eval=FALSE, class='sql'}
/***************************************
*
...
## [Data Visualization and Utilization in Tableau](https://public.tableau.com/app/profile/mustapha.bouhsen/viz/Stockprice_17081222957660/Dashboard1):

To address the limitation of not having **Power BI** for free Azure accounts and using directly SQL tables on it, I implemented a solution. I created a process that automatically downloads the table from Azure to my local machine at scheduled intervals using **crontab**. Subsequently, I leverage these locally stored tables in **Tableau Public** to create visualizations.

I have developed a **dynamic dashboard** for visualizing the data, integrating live updates sourced from automatically downloaded tables. You can access to this **dashboard** by clicking [here](https://public.tableau.com/app/profile/mustapha.bouhsen/viz/Stockprice_17081222957660/Dashboard1) and select mode **full screen**.